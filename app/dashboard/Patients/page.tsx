...
function statusStyle(status: string) {
  switch (status) {
    ...
    case "COMPLETED":         return "text-[#3DAA7A] border border-[#3DAA7A] bg-[#D1FAE5] text-[10px] font-bold px-2 py-0.5 rounded tracking-wide";
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg p-8">
        ...
        <div className="mb-6">
          ...
          <select
            ...
            className="px-3 py-2 rounded-lg border border-[#D9C9A8] text-sm text-[#3D0A1F] bg-white outline-none appearance-none"
          >
        ...
        <div className="flex justify-end gap-3">
          <button
            ...
            className="px-6 py-2.5 rounded-xl text-sm font-semibold text-white bg-[#591727]"
          >
            Save
          </button>
          <button
            onClick={onClose}
            className="px-6 py-2.5 rounded-xl text-sm font-semibold border border-[#D9C9A8] text-[#3D0A1F]"
          >
            Discard Changes
          </button>
        </div>
...
<div className="flex items-center gap-3 mb-6">
  ...
  <button
    onClick={onBack}
    className="w-9 h-9 flex items-center justify-center rounded-full hover:opacity-80 transition"
    style={{
  ...
  <div className="flex items-center gap-2">
    ...
    <span className="text-2xl font-bold" style={{ color: isDark ? "#B09070" : "#7A6040" }}>
      / Details
    </span>
  </div>
</div>
      ...
      <div className="flex gap-4">
        ...
        <div className="w-56 shrink-0 flex flex-col gap-4">
          ...
          <div className={`rounded-2xl p-6 border ${cardBorder}`} style={{ backgroundColor: card }}>
            <h2 className="text-[19px] font-bold mb-1   whitespace-nowrap" style={{ color: text1 }}>{patient.name}</h2>
            <p className="text-sm text-center mb-5" style={{ color: text2 }}>{patient.id}</p>
            <div className="flex flex-col gap-2 mb-4">
              <div className="flex items-center gap-2">
                <span className="text-md">✉</span>
                <span className="text-md whitespace-nowrap break-all" style={{ color: text2 }}>{patient.email}</span>
              </div>
              <div className="flex items-center gap-2">
  ...
  >
    <path d="M22 16.92v3a2 2 0 0 1-2.18 2 
             ...
             c.8.26 1.64.45 2.5.57A2 2 0 0 1 22 16.92z"/>
                ...
                <span className="text-md" style={{ color: text2 }}>{patient.phone}</span>
              </div>
            </div>
          </div>
            ...
            <div className="flex flex-col gap-2">
              {patient.previousVisits.map((v, i) => (
                <div key={i} className="flex items-center justify-between">
                  <span className="text-md" style={{ color: text2 }}>{v.specialty}</span>
                  <span className="text-xs font-semibold numeric-font" style={{ color: text1 }}>{v.date}</span>
                </div>
              ))}
              <div className="flex items-center justify-between mt-1 pt-2" style={{ borderTop: `1px solid ${isDark ? "#5C2A3A" : "#D9C9A8"}` }}>
                <span className="text-xs" style={{ color: text2 }}>Patient Since</span>
                <span className="text-xs font-semibold" style={{ color: text1 }}>{patient.patientSince}</span>
              </div>
            </div>
          </div>
          ...
          {/* Send Email Button */}
          <button
  className="w-full py-3 rounded-xl text-sm font-semibold font-seasons text-white flex items-center justify-center gap-2 transition-opacity hover:opacity-90"
...
>
  ...
  >
    ...
    <polygon points="22 2 15 22 11 13 2 9 22 2" />
        ...
        <div className="flex-1 flex flex-col gap-4">
          ...
          <div className={`rounded-2xl border ${cardBorder} flex items-center gap-6 overflow-hidden`} style={{ backgroundColor: card }}>
            {/* Date block */}
            <div
              className="flex flex-col items-center justify-center px-8 py-6 shrink-0 self-stretch"
              style={{ backgroundColor: isDark ? "#8B1A2E" : "#591727" }}
            >
              <span className="text-xs font-semibold text-white/70 mb-1">Upcoming</span>
              <span className="text-5xl font-bold text-white leading-none numeric-font">{patient.upcomingDate}</span>
              <span className="text-xs text-white/70 mt-1">{patient.upcomingMonth}</span>
            </div>
            {/* Service info */}
            <div className="flex-1 py-4">
              <h3 className="text-2xl font-bold mb-1" style={{ color: text1 }}>{patient.upcomingService}</h3>
              <p className="text-md mb-2" style={{ color: text2 }}>with {patient.upcomingDoctor}</p>
  ...
  >
    ...
    <polyline points="12 6 12 12 16 14" />
                ...
                <span className="text-md font-semibold numeric-font" style={{ color: text2 }}>{patient.upcomingTime}</span>
              </div>
            </div>
            {/* Status */}
            <div className="pr-6">
            ...
            <div className="flex border-b" style={{ borderColor: isDark ? "#5C2A3A" : "#D9C9A8" }}>
              {(["overview", "history", "notes"] as DetailTab[]).map(tab => (
                <button
                  ...
                  style={{ color: activeTab === tab ? (isDark ? "#591727" : "#591727") : text2 }}
                >
            ...
            {activeTab === "overview" && (
              <div className="p-6">
                ...
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-lg font-bold uppercase tracking-wide" style={{ color: text1 }}>Clinical Observations</h3>
                  <button
  onClick={() => setShowNewNote(true)}
  className="flex items-center gap-1 text-xs font-semibold"
  style={{ color: isDark ? "#D4A574" : "#591727" }}
>
  <svg
    width="14"
    height="14"
  ...
  <p
    ...
    style={{ color: isDark ? "#591727" : "#591727" }}
  >
    {patient.clinicalNoteDate}
  </p>
</div>
                  <p className="text-md " style={{ color: text1 }}>{patient.clinicalNote}</p>
                </div>

                {/* Recent Documents */}
                <h3 className="text-lg font-bold uppercase tracking-wide mb-4" style={{ color: text1 }}>Recent Documents</h3>
                <div className="flex flex-col gap-3 mb-6">
                  {patient.documents.length === 0 && (
                    <p className="text-sm" style={{ color: text2 }}>No documents uploaded.</p>
                  )}
                  {patient.documents.map((doc, i) => (
                    <div key={i} className={`flex items-center justify-between p-4 rounded-xl border ${cardBorder}`} style={{ backgroundColor: cardInner }}>
                      <div className="flex items-center gap-3">
                        <div
  className="w-9 h-9 rounded-lg flex items-center justify-center"
  style={{ backgroundColor: isDark ? "#6B2A40" : "#FEE2E2" }}
>
  {doc.icon === "img" ? (
    <svg
      width="16"
    ...
    >
      ...
      <line x1="10" y1="9" x2="8" y2="9" />
                        ...
                        <div>
                          <p className="text-md font-semibold" style={{ color: text1 }}>{doc.name}</p>
                          <p className="text-[14px]" style={{ color: text2 }}>{doc.added} · {doc.size}</p>
                        </div>
                      </div>
                      <button style={{ color: text2 }}>⬇</button>
                    </div>
                  ))}
                </div>
  ...
  style={{ backgroundColor: "#591727", color: "#F5ECD7" }}
>
  <svg
    width="12"
    height="12"
    ...
    <line x1="9" y1="14" x2="15" y2="14" />
                    ...
                    <div className="flex flex-wrap gap-2">
                      {patient.postOpInstructions.map((instr, i) => (
                        <span key={i} className="px-3 py-1 font-cinzel rounded-full text-[11px] font-semibold border" style={{ borderColor: isDark ? "#5C2A3A" : "#D9C9A8", color: text2, backgroundColor: "#d8bcbc" }}>
                          {instr}
                        </span>
                      ))}
                    </div>
            ...
            {activeTab === "history" && (
              <div className="p-6">
                <h3 className="text-lg font-bold mb-5" style={{ color: text1 }}>History</h3>
                  ...
                  <div className="flex flex-col gap-8 pl-8">
                    {patient.historyEntries.map((entry, i) => (
                      <div key={i} className="relative">
                        {/* Timeline dot */}
                        <div className="absolute -left-8 top-1 w-3.5 h-3.5 rounded-full border-2 border-white" style={{ backgroundColor: isDark ? "#8B1A2E" : "#591727" }} />
                        <div className="mb-2">
                        ...
                        <div className="flex flex-col gap-6 ml-19 ">
                          {/* Complaint */}
                          <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                              {entry.diagnostics.map((d, di) => (
                                <span key={di}
                                  ...
                                  style={{
                                    backgroundColor: d.tag ? "#ffffff" : cardInner,
                                    borderColor: d.tag ? (isDark ? "#C9922A" : "#C9922A") : undefined,
                                    color: d.tag ? "#C9922A" : text1,
                                  }}
                                >
                                  {d.label}
                                </span>
...
>
  ...
  <path d="M3 3h18v18H3z" />
            ...
            {activeTab === "notes" && (
              <div className="p-6">
                <div className="flex items-center justify-between mb-5">
                  <h3 className="text-lg font-bold uppercase tracking-wide" style={{ color: text1 }}>Notes</h3>
                  <button
  onClick={() => setShowNewNote(true)}
  className="flex items-center gap-1 text-xs font-semibold"
  style={{ color: isDark ? "#D4A574" : "#591727" }}
>
  ...
  New Note
                ...
                <div className="flex flex-col gap-4">
                  {notes.map((note, i) => (
                    <div key={i} className={`rounded-2xl p-5 border ${cardBorder}`} style={{ backgroundColor: cardInner }}>
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex items-center gap-3">
                          <div
                            className="w-10 h-10 rounded-full flex items-center justify-center text-xs font-bold shrink-0"
                            style={{ backgroundColor: isDark ? "#6B2A40" : "#8B5060", color: "#F5ECD7" }}
                          >
                            {note.doctorInitials}
                          </div>
                          <div>
                            <p className="text-sm font-bold" style={{ color: text1 }}>{note.doctor}</p>
                            <p className="text-xs numeric-font" style={{ color:"#2D0B14" }}>{note.date}</p>
                          </div>
                        </div>
                        <span className={statusStyle(note.status)}>{note.status}</span>
                      </div>
                      <p className="text-md mb-4" style={{ color: text2 }}>{note.content}</p>
                      <div className="flex gap-4">
                        <button
...
const Icon = ({ d }: { d: string }) => (
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-end">
      <div className="absolute inset-0 bg-black/40" onClick={onClose} />
      <div className="relative w-full max-w-2xl h-full overflow-y-auto p-8" style={{ backgroundColor: pageBg }}>
        <div className="flex items-center gap-3 mb-8">
          <h2 className="text-2xl font-bold" style={{ color: isDark ? "#ffffff" : "#591727" }}>PATIENTS /</h2>
          <span className="text-2xl font-bold" style={{ color: isDark ? "#B09070" : "#7A6040" }}>Add Checkup for {patient.name}</span>
        </div>
  ...
  <div key={f.label}>
    <label className="flex items-center gap-2 text-[13px] font-semibold numeric-font mb-2"
      style={{ color: "#681A2D" }}>
    ...
    <textarea
      ...
      style={{
        backgroundColor: isDark ? "#c1a694" : "#ffffff",
        borderColor: isDark ? "#5C2A3A" : "#D9C9A8",
        color: "#591727"
      }}
    />
  </div>
          ...
          <div className="flex justify-end gap-3 mt-6">
            <button onClick={onClose} className="px-6 py-2.5 rounded-xl text-sm font-semibold border" style={{ borderColor: isDark ? "#5C2A3A" : "#3D0A1F", color: text1 }}>
              Discard Changes
            </button>
            <button onClick={handleSave} className="px-6 py-2.5 rounded-xl text-sm font-semibold text-white" style={{ backgroundColor: isDark ? "#8B1A2E" : "#3D0A1F" }}>
              Save Checkup
            </button>
          </div>
        </div>
...
function CalendarDropdown({ onClose }: { onClose: () => void }) {
  ...
  const startDay = 1; // Monday offset
  
  return (
    <div className="absolute top-full right-0 left-0.2 mt-2 z-50 bg-white rounded-2xl shadow-2xl p-4 w-72" onClick={e => e.stopPropagation()}>
...
export default function PatientsPage() {
  ...
  // Color tokens — from the Appointments page code provided
  ...
  return (
    <div className="min-h-full ml-10 transition-colors duration-300" style={{ marginTop: "40px" }} onClick={() => { setShowCalendar(false); setShowStatusDrop(false); }}>
      ...
      {/* ── Page Header ──────────────────────────────────────── */}
      <h1 className="text-3xl font-bold tracking-wide mb-5" style={{ color: isDark ? "#ffffff" : "#591727" }}>
        PATIENTS
      </h1>
      ...
      <div className="flex gap-4 mb-6">
        {/* Total Patients */}
        <div
          className={`relative rounded-2xl p-6 border ${cardBorder} min-w-[300px]`}
        ...
        >
          ...
          <p className="text-md mt-1 " style={{ color: "#15803D" }}>↗ +12% from last month</p>
          {/* Capsule edge */}
          <div className="absolute right-0 top-1/2 -translate-y-1/2 h-[70%] w-[14px]">
            <div className="h-full w-full" style={{ backgroundColor: isDark ? "#5C2A3A" : "#ffe9bf", borderTopLeftRadius: "9999px", borderBottomLeftRadius: "9999px", boxShadow: "inset 4px 0 8px rgba(0,0,0,0.15)" }} />
          </div>
        </div>
      ...
      <div className={`rounded-2xl border ${cardBorder} overflow-hidden`} style={{ backgroundColor: tableBg }}>
        <div className="flex items-center gap-3 p-4 border-b" style={{ borderColor: isDark ? "#5C2A3A" : "#D9C9A8" }}>
          ...
          <div className="relative">
            ...
            >
              {statusFilter} <span className="text-xs">▾</span>
            </button>
            {showStatusDrop && (
              <StatusDropdown
          ...
          <thead>
            <tr style={{ borderBottom: `1px solid ${isDark ? "#5C2A3A" : "#D9C9A8"}` }}>
              {["Patient Name", "Email & Phone", "Spécialités", "Last Visit", "Next Appointment", "Status", "Actions"].map(h => (
                <th key={h} className="text-left px-4 py-3 text-[14px] font-semibold" style={{ color: text2 }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {filtered.map((row, i) => (
              ...
              >
                ...
                <td className="px-4 py-3">
                  <div className="text-[13px]" style={{ color: text2 }}>{row.email}</div>
                  <div className="text-[13px]" style={{ color: text2 }}>{row.phone}</div>
                </td>
                <td className="px-4 py-3">
                  <span className="text-[13px]" style={{ color: text2 }}>{row.specialty}</span>
                </td>
                <td className="px-4 py-3">
                  <span className="text-[13px]" style={{ color: text2 }}>{row.lastVisit}</span>
                </td>
                <td className="px-4 py-3">
                  {row.nextAppt === "None Scheduled" ? (
                    <span className="text-[13px]" style={{ color: text2 }}>None Scheduled</span>
                  ) : (
